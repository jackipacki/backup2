...
## Engagement Type — Confirm First

Before anything else, determine the mode. Same target, different rules:

| Mode | What counts as a finding | What gets rejected |
|------|--------------------------|-------------------|
| **Bug bounty** (H1/Bugcrowd/Intigriti) | Impact-demonstrated bugs ONLY. Full chain to attacker-attainable harm. | Hygiene (EoL software, permissive CSP, stack traces without impact, "best practice" violations) |
| **Red team** (external client) | Hygiene + recon + IoCs + defensive observations are ALL deliverables | Nothing — even "no finding here" is reportable |
| **Pentest** (signed SoW/WAPT) | Depends on SoW. Usually hygiene + impact + recon | Out-of-scope assets |
| **Internal audit** | Compliance-mapped findings (PCI/ISO/NIST/GDPR) | Findings without a control-mapping |

**Hard rule:** Write engagement type as the first line in hunt notes. If unclear, ask once.
...
Run the 7-Question Gate (read `03-triage-validation.md`). One wrong answer = KILL.
