...
### Online/Offline status indicator (Optional)[​](#onlineoffline-status-indicator-optional "Direct link to Online/Offline status indicator (Optional)")
...
```
gateway:   platforms:     telegram:       extra:         status_indicator:  true         
# Optional custom strings (defaults: "Online" / "Offline"):         status_online:  "🟢 Online"         status_offline:  "🔴 Offline"
```
...
### Command menu priority and cap (Optional)[​](#command-menu-priority-and-cap-optional "Direct link to Command menu priority and cap (Optional)")
...
```
platforms:   telegram:     extra:       command_menu:         max_commands:  60         priority_mode:  prepend  
# prepend | append | replace         priority:           -  my_plugin_command
```
...
### Observe group chatter without auto-replying[​](#observe-group-chatter-without-auto-replying "Direct link to Observe group chatter without auto-replying")
...
```
TELEGRAM_ALLOWED_CHATS = -1001234567890 TELEGRAM_GROUP_ALLOWED_CHATS = -1001234567890 TELEGRAM_OBSERVE_UNMENTIONED_GROUP_MESSAGES = true
```
...
### Supported `MEDIA:` file extensions[​](#supported-media-file-extensions "Direct link to supported-media-file-extensions")
...
```
TELEGRAM_WEBHOOK_URL =https://my-app.fly.dev/telegram TELEGRAM_WEBHOOK_SECRET = "$(openssl rand -hex  32) "   
# required 
...
```
fly secrets set  TELEGRAM_WEBHOOK_URL =https://my-app.fly.dev/telegram fly secrets set  TELEGRAM_WEBHOOK_SECRET =$(openssl rand -hex  32)
```
...
### Cron deliveries in topic mode[​](#cron-deliveries-in-topic-mode "Direct link to Cron deliveries in topic mode")
...
#### Skipping STT: pass the raw audio file to the agent[​](#skipping-stt-pass-the-raw-audio-file-to-the-agent "Direct link to Skipping STT: pass the raw audio file to the agent")
...
## Large Files (>20MB) via Local Bot API Server[​](#large-files-20mb-via-local-bot-api-server "Direct link to Large Files (>20MB) via Local Bot API Server")
...
### Step 1: Obtain Telegram API credentials[​](#step-1-obtain-telegram-api-credentials "Direct link to Step 1: Obtain Telegram API credentials")
...
### Step 2: Run the telegram-bot-api server[​](#step-2-run-the-telegram-bot-api-server "Direct link to Step 2: Run the telegram-bot-api server")
...
```
services:   tg-bot-api:     image: aiogram/telegram - bot - api: latest     container_name:  tg - bot - api     restart:  unless - stopped     ports:       -  "127.0.0.1:8081:8081"    
# bind to loopback only; see security note     environment:       TELEGRAM_API_ID:  "12345"            
# your api_id from Step 1       TELEGRAM_API_HASH:  "abcdef..."      
# your api_hash from Step 1       TELEGRAM_LOCAL:  "1"                 
# enable --local mode (raises 20MB → 2GB)     volumes:       - ./tg - bot - api - data:/var/lib/telegram - bot - api
```

Bring it up:

```
docker  compose up -d tg-bot-api docker  logs --tail  20 tg-bot-api
```
...
### Step 3: Log the bot out of the public API (one-time)[​](#step-3-log-the-bot-out-of-the-public-api-one-time "Direct link to Step 3: Log the bot out of the public API (one-time)")
...
### Step 4: Point Hermes at the local server[​](#step-4-point-hermes-at-the-local-server "Direct link to Step 4: Point Hermes at the local server")
...
# see Step 5 below — only set this if the bot's data                               
...
```
hermes gateway restart grep  -E  "Using custom Telegram base_url|Using Telegram local_mode" ~/.hermes/logs/gateway.log |  tail
```

### Step 5: `local_mode` — file access on disk[​](#step-5-local_mode--file-access-on-disk "Direct link to step-5-local_mode--file-access-on-disk")
...
### Step 6: Test it[​](#step-6-test-it "Direct link to Step 6: Test it")
...
### Multiple Hermes bots in one group[​](#multiple-hermes-bots-in-one-group "Direct link to Multiple Hermes bots in one group")
...
```
# default profile hermes gateway start hermes gateway status hermes gateway stop 
# named profiles hermes -p  research gateway start hermes -p  research gateway status hermes -p  research gateway stop
```
...
### Troubleshooting: works in DMs but not groups[​](#troubleshooting-works-in-dms-but-not-groups "Direct link to Troubleshooting: works in DMs but not groups")
...
```
platforms:   telegram:     extra:       dm_topics:       -  chat_id:  123456789         
# Your Telegram user ID         topics:         -  name:  General           icon_color:  7322096         -  name:  Website           icon_color:  9367192         -  name:  Research           icon_color:  16766590           skill:  arxiv              
...
### Root DM handling[​](#root-dm-handling "Direct link to Root DM handling")
...
## Multi-session DM mode (`/topic`)[​](#multi-session-dm-mode-topic "Direct link to multi-session-dm-mode-topic")
...
### `/topic` subcommands[​](#topic-subcommands "Direct link to topic-subcommands")
...
### DM Topics vs Multi-session DM mode[​](#dm-topics-vs-multi-session-dm-mode "Direct link to DM Topics vs Multi-session DM mode")
...
### Activation flow[​](#activation-flow "Direct link to Activation flow")
...
### Creating a new topic (end-user flow)[​](#creating-a-new-topic-end-user-flow "Direct link to Creating a new topic (end-user flow)")
...
### Auto-renamed topics[​](#auto-renamed-topics "Direct link to Auto-renamed topics")
...
### `/new` inside a topic[​](#new-inside-a-topic "Direct link to new-inside-a-topic")
...
### Restoring a previous session[​](#restoring-a-previous-session "Direct link to Restoring a previous session")
...
### `/topic` inside a topic (no argument)[​](#topic-inside-a-topic-no-argument "Direct link to topic-inside-a-topic-no-argument")
...
* The General (pinned top) topic in a forum-enabled DM is treated as the root lobby, regardless of whether Telegram delivers its messages with `message_thread_id=1` or with no thread\_id
...
### Disabling multi-session mode[​](#disabling-multi-session-mode "Direct link to Disabling multi-session mode")
...
### Downgrading Hermes[​](#downgrading-hermes "Direct link to Downgrading Hermes")
...
```
platforms:   telegram:     extra:       group_topics:       -  chat_id:  -1001234567890        
# Supergroup ID         topics:         -  name:  Engineering           thread_id:  5           skill:  software - development         -  name:  Research           thread_id:  12           skill:  arxiv         -  name:  General           thread_id:  1           
...
### Streaming transport (`gateway.streaming.transport`)[​](#streaming-transport-gatewaystreamingtransport "Direct link to streaming-transport-gatewaystreamingtransport")
...
```
gateway:   streaming:     enabled:  true     transport:  auto    
# auto | draft | edit | off
```
...
## Rendering: Rich Messages, Tables and Link Previews[​](#rendering-rich-messages-tables-and-link-previews "Direct link to Rendering: Rich Messages, Tables and Link Previews")
...
## Group Allowlisting[​](#group-allowlisting "Direct link to Group Allowlisting")
...
# Global access (DMs + groups). Users here can always invoke the bot.         allow_from:           -  "123456789"         
# Sender IDs allowed in groups/forums only. Does NOT grant DM access.         group_allow_from:           -  "987654321"         
# Entire groups/forums — any member is authorized.         group_allowed_chats:           -  "-1001234567890"
```
...
### Migration from before PR #17686[​](#migration-from-before-pr-17686 "Direct link to Migration from before PR #17686")
...
```
# Old (still works, but deprecated)  TELEGRAM_GROUP_ALLOWED_USERS ="-1001234567890"  
# New  TELEGRAM_GROUP_ALLOWED_CHATS ="-1001234567890"
```

### Guest @mention bypass (`guest_mode`)[​](#guest-mention-bypass-guest_mode "Direct link to guest-mention-bypass-guest_mode")
...
```
gateway:   platforms:     telegram:       extra:         group_allowed_chats:           -  "-1001234567890"    
# your main allowlisted group         guest_mode:  true        
# non-allowlisted groups: allow on @mention only
```
...
## Slash Command Access Control[​](#slash-command-access-control "Direct link to Slash Command Access Control")
...
```
gateway:   platforms:     telegram:       extra:         
# Existing allowlists (unchanged)         allow_from:           -  "123456789"      
# admin           -  "555555555"      
# regular user           -  "777777777"      
# regular user          
# NEW — admins get all slash commands (built-in + plugin)         allow_admin_from:           -  "123456789"          
# NEW — non-admin allowed users can only run these slash commands.         
# /help and /whoami are always allowed so users can see their access.         user_allowed_commands:           -  status           -  model           -  history         
# Optional: separate admin/command lists for groups         group_allow_admin_from:           -  "123456789"         group_user_allowed_commands:           -  status
```
...
```
export  HTTPS_PROXY =http://proxy.example.com:8080 hermes gateway
```
...
## Interactive Prompts (clarify)[​](#interactive-prompts-clarify "Direct link to Interactive Prompts (clarify)")
...
## Push notification volume[​](#push-notification-volume "Direct link to Push notification volume")
...
## Status messages edited in place[​](#status-messages-edited-in-place "Direct link to Status messages edited in place")
...
## Pin incoming user message during agent turn[​](#pin-incoming-user-message-during-agent-turn "Direct link to Pin incoming user message during agent turn")
...
* [Step 2: Customize Your Bot (Optional)](#step-2-customize-your-bot-optional)
  + [Online/Offline status indicator (Optional)](#onlineoffline-status-indicator-optional)
  + [Command menu priority and cap (Optional)](#command-menu-priority-and-cap-optional)
* [Step 3: Privacy Mode (Critical for Groups)](#step-3-privacy-mode-critical-for-groups)
  ...
  + [Observe group chatter without auto-replying](#observe-group-chatter-without-auto-replying)
...
* [Sending Generated Files from Docker-backed Terminals](#sending-generated-files-from-docker-backed-terminals)
  + [Supported `MEDIA:` file extensions](#supported-media-file-extensions)
...
* [Home Channel](#home-channel)
  + [Cron deliveries in topic mode](#cron-deliveries-in-topic-mode)
...
* [Large Files (>20MB) via Local Bot API Server](#large-files-20mb-via-local-bot-api-server)
  + [Step 1: Obtain Telegram API credentials](#step-1-obtain-telegram-api-credentials)
  + [Step 2: Run the telegram-bot-api server](#step-2-run-the-telegram-bot-api-server)
  + [Step 3: Log the bot out of the public API (one-time)](#step-3-log-the-bot-out-of-the-public-api-one-time)
  + [Step 4: Point Hermes at the local server](#step-4-point-hermes-at-the-local-server)
  + [Step 5: `local_mode` — file access on disk](#step-5-local_mode--file-access-on-disk)
  + [Step 6: Test it](#step-6-test-it)
* [Group Chat Usage](#group-chat-usage)
  + [Multiple Hermes bots in one group](#multiple-hermes-bots-in-one-group)
  + [Troubleshooting: works in DMs but not groups](#troubleshooting-works-in-dms-but-not-groups)
...
* [Private Chat Topics (Bot API 9.4)](#private-chat-topics-bot-api-94)
  ...
  + [Root DM handling](#root-dm-handling)
  + [Skill binding](#skill-binding)
* [Multi-session DM mode (`/topic`)](#multi-session-dm-mode-topic)
  + [`/topic` subcommands](#topic-subcommands)
  + [DM Topics vs Multi-session DM mode](#dm-topics-vs-multi-session-dm-mode)
  + [Prerequisites](#prerequisites)
  + [Activation flow](#activation-flow)
  + [Creating a new topic (end-user flow)](#creating-a-new-topic-end-user-flow)
  + [Auto-renamed topics](#auto-renamed-topics)
  + [`/new` inside a topic](#new-inside-a-topic)
  + [Restoring a previous session](#restoring-a-previous-session)
  + [`/topic` inside a topic (no argument)](#topic-inside-a-topic-no-argument)
  + [Under the hood](#under-the-hood)
  + [Disabling multi-session mode](#disabling-multi-session-mode)
  + [Downgrading Hermes](#downgrading-hermes)
...
* [Rendering: Rich Messages, Tables and Link Previews](#rendering-rich-messages-tables-and-link-previews)
* [Group Allowlisting](#group-allowlisting)
  + [Migration from before PR #17686](#migration-from-before-pr-17686)
  + [Guest @mention bypass (`guest_mode`)](#guest-mention-bypass-guest_mode)
* [Slash Command Access Control](#slash-command-access-control)
...
* [Push notification volume](#push-notification-volume)
* [Status messages edited in place](#status-messages-edited-in-place)
* [Pin incoming user message during agent turn](#pin-incoming-user-message-during-agent-turn)
* [Security](#security)